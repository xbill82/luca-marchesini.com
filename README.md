...
One of the main needs I had was optimization, both for Js, CSS and images. When optimization comes into play, most of the time I end up throwing away all the automated steps of the project to go and tweak things manually in order to achieve minimum file size and best compression. But now things are different. [R.js](http://requirejs.org/docs/optimization.html) optimizer is perfectly integrated with Grunt and can silently run in my `build` task. Same thing for the [LESS](http://lesscss.org/) compiler. Which is just *great*.
